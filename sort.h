...
// 插入排序（降序）- 小规模数据
template <typename T>
void insertionSort(SeqList<T>& arr, bool (*comp)(const T& a, const T& b)) {
    int n = arr.getSize();
    for (int i = 1; i < n; i++) {
...
// 快速排序（降序）- 中等规模数据
template <typename T>
int partition(SeqList<T>& arr, int left, int right, bool (*comp)(const T& a, const T& b)) {
...
template <typename T>
void quickSort(SeqList<T>& arr, int left, int right, bool (*comp)(const T& a, const T& b)) {
    if (left < right) {
        int pivotIdx = partition(arr, left, right, comp);
...
// 堆排序（降序）- 大规模数据
template <typename T>
void heapify(SeqList<T>& arr, int n, int i, bool (*comp)(const T& a, const T& b)) {
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
...
template <typename T>
void heapSort(SeqList<T>& arr, bool (*comp)(const T& a, const T& b)) {
    int n = arr.getSize();
...
template <typename T>
void ShiyingSort(SeqList<T>& arr, bool (*comp)(const T& a, const T& b)) {
    int n = arr.getSize();
    if (n <= 1) return;
